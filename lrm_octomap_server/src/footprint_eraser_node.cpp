...
tf::StampedTransform transform_min;
tf::StampedTransform transform_max;
...
void odomCallback(const nav_msgs::Odometry::ConstPtr & msg) {
	...
	*/
	octomath::Vector3 pt1(last_pose.pose.position.x, last_pose.pose.position.y, last_pose.pose.position.z);
	...
	if (dist > _min_distance_to_update) {
		...
		tf::Transform trans_odom(qt, tf::Vector3(
		...
		trans_min = trans_odom * transform_min;
		trans_max = trans_odom * transform_max;
		...
		ROS_DEBUG_STREAM("call service (" << min.x << ":" << min.y << ":" << min.z << ")("
				<< max.x << ":" << max.y << ":" << max.z << ")");
...
int main(int argc, char **argv) {
	...
	nh_priv.param<double>("min_distance", _min_distance_to_update, 0.8);
	...
	try {
		listener.lookupTransform("base_footprint", "bbx_base_rear_left_link", ros::Time(0), transform_min);
	} catch (tf::TransformException &ex) {
		ROS_ERROR("footprint_eraser: %s", ex.what());
	}

	try {
		listener.lookupTransform("base_footprint", "bbx_top_front_right_link", ros::Time(0), transform_max);
	} catch (tf::TransformException &ex) {
		ROS_ERROR("footprint_eraser: %s", ex.what());
	}
