...
public:
	...
	void starting() {
		ROS_WARN_STREAM("Suspension controller starting...");
	}

	void stopping() {
		ROS_WARN_STREAM("Suspension controller stopping...");
	}
...
private:
	...
	bool loadJoints(ros::NodeHandle& n_) {
		...
		ROS_WARN_STREAM("SuspensionController tf_prefix:" << tf_prefix_ << " ns:" << n_.getNamespace());
		...
		/*
		 if(n_.haveParam("effort")) {
		 n_.getParam<double>("effort", effort_);
		 }
		 else {
		 effort_ = 1;
		 }
...
/*
 ...
 lrm_gazebo,
 SuspensionControllerPlugin,
 lrm_gazebo::SuspensionController,
