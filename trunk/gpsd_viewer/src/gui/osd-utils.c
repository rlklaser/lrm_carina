...
#include "gpsd_viewer/gui/osd-utils.h"
...
static void debug_bbox(cairo_t *cr, gint x, gint y, gint w, gint h) {
#if DEBUG_DRAWING
...
void osd_render_crosshair_shape(cairo_t *cr, int w, int h, int r, int tick) {
	cairo_arc(cr, w / 2, h / 2, r, 0, 2 * M_PI);

	cairo_move_to(cr, w / 2 - r, h / 2);
	cairo_rel_line_to(cr, -tick, 0);
	cairo_move_to(cr, w / 2 + r, h / 2);
	cairo_rel_line_to(cr, tick, 0);

	cairo_move_to(cr, w / 2, h / 2 - r);
...
static void osd_zoom_shape(cairo_t *cr, gint x, gint y, gint w, gint h) {
...
static void osd_zoom_labels(cairo_t *cr, gint x, gint y, gint w, gint h) {
	gint r = h / 2; /* radius of curved ends of zoom pad */
...
static void osd_dpad_shape(cairo_t *cr, gint x, gint y, gint r) {
	cairo_arc(cr, x + r, y + r, r, 0, 2 * M_PI);
}

static void osd_dpad_labels(cairo_t *cr, gint x, gint y, gint r) {
...
void osd_render_gps(cairo_t *cr, gint x, gint y, gint w, GdkColor *bg, GdkColor *fg) {
