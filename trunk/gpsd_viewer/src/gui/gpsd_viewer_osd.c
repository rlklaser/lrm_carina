 ...
 */
enum _GPSD_VIEWER_OSD_PROPERTY_ID {
	PROP_0, PROP_OSD_X, PROP_OSD_Y, PROP_DPAD_RADIUS, PROP_SHOW_SCALE, PROP_SHOW_COORDINATES, PROP_SHOW_DPAD, PROP_SHOW_ZOOM, PROP_SHOW_GPS_IN_DPAD, PROP_SHOW_GPS_IN_ZOOM
...
static void gpsd_viewer_osd_class_init(GpsdViewerOsdClass *klass) {
	...
	g_object_class_install_property(object_class, PROP_OSD_X, g_param_spec_int("osd-x", "osd-x", "osd-x", G_MININT, G_MAXINT, 10, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
	g_object_class_install_property(object_class, PROP_OSD_Y, g_param_spec_int("osd-y", "osd-y", "osd-y", G_MININT, G_MAXINT, 10, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
	g_object_class_install_property(object_class, PROP_DPAD_RADIUS, g_param_spec_uint("dpad-radius", "dpad-radius", "dpad radius", 0, G_MAXUINT, 30, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
	g_object_class_install_property(object_class, PROP_SHOW_SCALE, g_param_spec_boolean("show-scale", "show-scale", "show scale on the map", TRUE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
	g_object_class_install_property(object_class, PROP_SHOW_COORDINATES, g_param_spec_boolean("show-coordinates", "show-coordinates", "show coordinates of map centre", TRUE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
	g_object_class_install_property(object_class, PROP_SHOW_DPAD, g_param_spec_boolean("show-dpad", "show-dpad", "show dpad for map navigation", FALSE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
	g_object_class_install_property(object_class, PROP_SHOW_ZOOM, g_param_spec_boolean("show-zoom", "show-zoom", "show zoom control for map navigation", FALSE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
	g_object_class_install_property(object_class, PROP_SHOW_GPS_IN_DPAD, g_param_spec_boolean("show-gps-in-dpad", "show-gps-in-dpad", "show gps indicator in middle of dpad", FALSE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
	g_object_class_install_property(object_class, PROP_SHOW_GPS_IN_ZOOM, g_param_spec_boolean("show-gps-in-zoom", "show-gps-in-zoom", "show gps indicator in middle of zoom control", FALSE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}
...
static void scale_render(GpsdViewerOsd *self, OsmGpsMap *map) {
	...
	cairo_stroke(cr);
	cairo_move_to(cr, 2 * GPSD_VIEWER_OSD_SCALE_FD, GPSD_VIEWER_OSD_SCALE_H2 + GPSD_VIEWER_OSD_SCALE_FD + extents.height);
...
static void scale_draw(GpsdViewerOsd *self, GtkAllocation *allocation, cairo_t *cr) {
	...
	if (x < 0)
		x += allocation->width - GPSD_VIEWER_OSD_SCALE_W;
	if (y < 0)
		y += allocation->height - GPSD_VIEWER_OSD_SCALE_H;
...
static void drone_render(GpsdViewerOsd *self, OsmGpsMap *map) {
	...
	if (!drone->surface || !drone->need_render) {
		...
		pat = cairo_pattern_create_radial(GPSD_VIEWER_OSD_UAV_W / 2 - 1, GPSD_VIEWER_OSD_COORDINATES_H / 2 - 1, GPSD_VIEWER_OSD_UAV_RADIUS * 0.2, GPSD_VIEWER_OSD_UAV_W / 2, GPSD_VIEWER_OSD_COORDINATES_H / 2, GPSD_VIEWER_OSD_UAV_RADIUS);
...
static void coordinates_render(GpsdViewerOsd *self, OsmGpsMap *map) {
	...
	if (!isnan(coordinates->lat) && !isnan(coordinates->lon))
		...
		if ((fabsf(lat - coordinates->lat) < 1 / 60000) && (fabsf(lon - coordinates->lon) < 1 / 60000))
			return;
...
static void coordinates_draw(GpsdViewerOsd *self, GtkAllocation *allocation, cairo_t *cr) {
	...
	y = -priv->gpsd_viewer_osd_y;
	if (x < 0)
		x += allocation->width - GPSD_VIEWER_OSD_COORDINATES_W;
	if (y < 0)
		y += allocation->height - GPSD_VIEWER_OSD_COORDINATES_H;
