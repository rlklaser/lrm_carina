...
std::string _utm_zone;

void gpsFixCallback (const  geometry_msgs::PoseStampedPtr pose)
{
  ...
  gps_common::UTMtoLL(pose->pose.position.y, pose->pose.position.x, _utm_zone.c_str(), latitude, longitude);
  ...
  if (OSM_IS_GPS_MAP (data->map)){
		...
		if(data->lock_view)
		{
			...
			osm_gps_map_set_center(data->map, latitude, longitude);
		}
		else
		{
...
void *startROS (void *user)
{
  if (user != NULL)
  {
    ...
    ros::init (p_arg->argc, p_arg->argv, "gpsd_pose_viewer");
    ros::NodeHandle n;
    ...
    n_param.param("utm_zone", _utm_zone, std::string("23K"));
    ...
    // -----------------------------------------------------------------      
    // **** topics subscribing
	ros::Subscriber fix_sub = n.subscribe ("pose", 1, gpsFixCallback);
...
int main (int argc, char **argv)
{
  ...
  OsmGpsMapPoint ccny_coord = { -22.0030, -47.9315 };
