...
#include <lrm_carina_description/Constants.h>
...
public:
	...
	void update() {
		...
		if (d.toNSec() > (1000000000.0 / update_rate_)) {
			...
			if (abs(encWheel) > WHEEL_ENCODER_RESOLUTION) {
				ROS_ERROR_STREAM("wheel encoder miss" << encWheel);
				encWheel = 0;
			}
...
private:
	...
	bool loadJoints(ros::NodeHandle& n_) {
		...
		ROS_INFO_STREAM("EncodersController tf_prefix:" << tf_prefix_ << " ns:" << n_.getNamespace());
...
PLUGINLIB_EXPORT_CLASS(lrm_gazebo::EncodersController, pr2_controller_interface::Controller)
