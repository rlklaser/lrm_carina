...
int main(int argc, char** argv) {
	...
	nh_priv.param("rate", rate, 20.0);
	nh_priv.param("tread_lenght", tread_lenght, 1.508);

	wheel_encoder = 0;

	ros::Subscriber encoders_sub = nh.subscribe<lrm_msgs::Encoders>("encoders", 1, &encodersCallback);
	ros::Publisher states_pub = nh.advertise<lrm_msgs::VehicleState>("vehicle_states", 1);
	...
	while (ros::ok()) {
		...
		double dist = (tread_lenght / WHEEL_ENCODER_RESOLUTION) * wheel_encoder;

		wheel_encoder = 0;

		double vel = dist / dt.toSec();
